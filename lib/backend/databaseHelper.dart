...
class DatabaseHelper {
    ...
    _db = await openDatabase(
      ...
      onUpgrade: (db, oldVersion, newVersion) async {
        ...
        if (oldVersion < 4) {
          ...
          await db.execute('ALTER TABLE posts ADD COLUMN commentCount INTEGER NOT NULL DEFAULT 0');
          ...
          await db.execute('ALTER TABLE comments ADD COLUMN parentCommentId INTEGER');
          // Notifications table
          await db.execute("""
            CREATE TABLE IF NOT EXISTS notifications (
              id           INTEGER PRIMARY KEY AUTOINCREMENT,
  ...
  Future<int> insertUser({
    ...
    required String password, String? email, String? profilePic,
  }) async {
  ...
  Future<List<Map<String, dynamic>>> getTopHelpers() async {
    ...
    return await db.query('users',
        columns: ['id','fullName','userName','points','helpCount','streak','profilePic'],
  ...
  Future<List<Map<String, dynamic>>> getSkillCards(int userId) async {
    ...
    return await db.query('skill_cards',
        where: 'userId = ?', whereArgs: [userId], orderBy: 'skillPoints DESC');
  }
  ...
  Future<int> insertPost({
    ...
    required String description, required String postType,
    required String category,
    String status = 'Open', String urgencyLevel = 'Low', String? imageUrl,
  }) async {
    final db     = await _database();
    ...
    return await db.insert('posts', {
      ...
      'urgencyLevel': urgencyLevel, 'isBoosted': 0, 'expiryTime': expiry,
      'commentCount': 0, 'synced': 0,
  ...
  Future<int> updatePost({
    required int postId, required String title,
    required String description, required String postType,
    required String category, required String status,
    String urgencyLevel = 'Low', String? imageUrl,
  }) async {
    final db = await _database();
  ...
  Future<int> updatePostStatus(int postId, String status, {
  ...
  Future<int> getCommentCount(int postId) async {
    ...
    final r  = await db.rawQuery(
        'SELECT COUNT(*) AS c FROM comments WHERE postId = ?', [postId]);
  ...
  Future<List<Map<String, dynamic>>> getCommentsByPost(int postId) async {
    ...
    for (final row in flat) {
      ...
      if (parent == null) {
        topLevel.add(Map<String, dynamic>.from(row)..['replies'] = <Map<String, dynamic>>[]);
      } else {
        replies.putIfAbsent(parent, () => []).add(Map<String, dynamic>.from(row));
      }
    }
  ...
  Future<int> deleteComment(int commentId) async {
    ...
    final rows = await db.query('comments',
        columns: ['postId'], where: 'id = ?', whereArgs: [commentId]);
    final result = await db.delete('comments',
        where: 'id = ?', whereArgs: [commentId]);
