  ...
  void paint(Canvas canvas, Size size) {
    canvas.drawPath(
      Path()
        ...
        ..lineTo(0, size.height * 0.20)..close(),
      Paint()..color = _amber.withOpacity(0.15),
    );
    canvas.drawPath(
      Path()..moveTo(0, 0)..lineTo(size.width * 0.22, 0)
        ..lineTo(0, size.height * 0.09)..close(),
      Paint()..color = _amber.withOpacity(0.09),
    );
    canvas.drawArc(
      Rect.fromCircle(center: Offset(size.width, 0), radius: size.width * 0.55),
      pi / 2, pi / 2, true, Paint()..color = _teal.withOpacity(0.10),
    );
    canvas.drawPath(
      Path()
    ...
    for (int row = 0; row < 5; row++)
      for (int col = 0; col < 5; col++)
        canvas.drawCircle(
            Offset(size.width - 24.0 - col * 16, 24.0 + row * 16), 2.0, dotPaint);
    final dotPaint2 = Paint()..color = _amber.withOpacity(0.14);
    for (int row = 0; row < 4; row++)
      for (int col = 0; col < 3; col++)
        canvas.drawCircle(
            Offset(18.0 + col * 16, size.height * 0.42 + row * 16), 1.8, dotPaint2);
    canvas.drawLine(
      Offset(size.width * 0.0, size.height * 0.30),
      Offset(size.width * 0.55, size.height * 0.0),
      Paint()..color = _amber.withOpacity(0.07)..style = PaintingStyle.stroke..strokeWidth = 22,
    );
    canvas.drawCircle(Offset(size.width * 0.85, size.height * 0.48),
        size.width * 0.30, Paint()..color = _teal.withOpacity(0.04));
  }
  @override
...
class _CommunityRingPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    canvas.drawCircle(Offset(cx, cy), size.width / 2 - 1,
        Paint()..color = _amber.withOpacity(0.20)..style = PaintingStyle.stroke..strokeWidth = 2);
    canvas.drawCircle(Offset(cx, cy), size.width / 2 - 9,
        Paint()..color = _amber.withOpacity(0.10));
    for (int i = 0; i < 6; i++) {
      final angle = (pi / 3) * i;
      canvas.drawCircle(
        Offset(cx + (size.width / 2 - 1) * cos(angle),
            cy + (size.width / 2 - 1) * sin(angle)),
        3.5, Paint()..color = _amber.withOpacity(0.55),
      );
    }
  }
  @override
...
class SignUpScreen extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return MaterialApp(debugShowCheckedModeBanner: false, home: SignUpScreenHome());
  }
}
...
class _SignUpScreenHomeState extends State<SignUpScreenHome>
  ...
  final emailController           = TextEditingController(); // ← Added: SRS users table has email
  final newPasswordController     = TextEditingController();
  final confirmPasswordController = TextEditingController();

  final _nameFocus    = FocusNode();
  final _userFocus    = FocusNode();
  final _emailFocus   = FocusNode();                         // ← Added
  ...
  // ── Input validation + account creation ──────────────────────────────────
  void inputValidations() async {
    if (fullNameController.text.trim().isEmpty) {
      _showError("Full name is required"); return;
    }
    if (userNameController.text.trim().isEmpty) {
      _showError("Username is required"); return;
    }
    ...
    if (newPasswordController.text.isEmpty) {
      _showError("Password is required"); return;
    }
    ...
    final result = await DatabaseHelper().insertUser(
      ...
      email: email.isNotEmpty ? email : null,   // ← passes email to DB
    );
    ...
    if (result > 0) {
      AwesomeDialog(
        ...
        dialogType: DialogType.success, btnOkColor: _amber,
  ...
  void _showError(String message) {
    AwesomeDialog(width: 300, context: context, title: 'Oops!',
        desc: message, dialogType: DialogType.error,
        btnOkColor: _amber, btnOkOnPress: () {}).show();
  }
                ...
                child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                  ...
                  GestureDetector(
                    ...
                    child: Container(
                      ...
                      decoration: BoxDecoration(
                        ...
                        boxShadow: [BoxShadow(color: _amber.withOpacity(0.12),
                        ...
                        children: [
                          RichText(text: const TextSpan(children: [
                            TextSpan(text: "Join\n", style: TextStyle(fontSize: 38,
                                fontWeight: FontWeight.w900, color: _inkDark,
                                height: 1.1, letterSpacing: -1.0)),
                            TextSpan(text: "Peer", style: TextStyle(fontSize: 38,
                                fontWeight: FontWeight.w900, color: _inkDark,
                                height: 1.1, letterSpacing: -1.0)),
                            TextSpan(text: "Aid", style: TextStyle(fontSize: 38,
                          ...
                          Container(
                            ...
                            decoration: BoxDecoration(color: _teal.withOpacity(0.10),
                                borderRadius: BorderRadius.circular(20)),
                            child: const Text("✨  Your campus community awaits",
                                style: TextStyle(color: _teal, fontSize: 11.5,
                                    fontWeight: FontWeight.w600)),
                          ),
                        ])),
                    CustomPaint(
                      painter: _CommunityRingPainter(), size: const Size(64, 64),
                      child: const SizedBox(width: 64, height: 64,
                          child: Center(child: Text("🤝", style: TextStyle(fontSize: 26)))),
                    ),
                  ]),

                  Container(height: 3, width: 52,
                  ...
                  Container(
                    decoration: BoxDecoration(
                      ...
                      boxShadow: [
                        BoxShadow(color: _amber.withOpacity(0.08),
                    ...
                    child: Column(children: [
                      ...
                      Row(children: [
                        ...
                        const Text("Personal Info", style: TextStyle(color: _inkDark,
                            fontSize: 13, fontWeight: FontWeight.w700,
                            letterSpacing: 0.2)),
                      ]),
                      const SizedBox(height: 16),
                      _buildField(controller: fullNameController, focusNode: _nameFocus,
                          hint: "e.g. Maria Santos", label: "Full Name",
                          icon: Icons.badge_rounded),
                      const SizedBox(height: 14),
                      _buildField(controller: userNameController, focusNode: _userFocus,
                          hint: "e.g. maria_s", label: "Username",
                          icon: Icons.alternate_email_rounded),
                      const SizedBox(height: 14),

                      // ── Email field (added per SRS users table) ──
                      _buildField(controller: emailController, focusNode: _emailFocus,
                          hint: "e.g. maria@school.edu", label: "Email (optional)",
                      ...
                      Row(children: [
                        ...
                        const SizedBox(width: 10),
                        const Text("Security", style: TextStyle(color: _inkDark,
                            fontSize: 13, fontWeight: FontWeight.w700,
                            letterSpacing: 0.2)),
                      ]),
                      const SizedBox(height: 16),
                      _buildField(controller: newPasswordController, focusNode: _passFocus,
                          hint: "Create a strong password", label: "Password",
                          icon: Icons.lock_rounded, obscure: hideNewPassword,
                      ...
                      _buildField(controller: confirmPasswordController,
                          ...
                          hint: "Repeat your password", label: "Confirm Password",
                          icon: Icons.lock_outline_rounded, obscure: hideConfirmPassword,
                          suffix: GestureDetector(
                  ...
                  SizedBox(
                    ...
                    child: DecoratedBox(
                      decoration: BoxDecoration(
                        ...
                        boxShadow: [BoxShadow(color: _amber.withOpacity(0.42),
                  ...
                  Center(
                    child: GestureDetector(
                      ...
                      child: RichText(text: const TextSpan(
                        ...
                        style: TextStyle(color: _inkMid, fontSize: 13.5),
                  ...
                  const SizedBox(height: 20),
                  Center(child: Text("PeerAid © 2025 · Campus Peer Support",
                      style: TextStyle(color: _inkLight.withOpacity(0.7),
  ...
  Widget _buildField({
    ...
    return Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
      Text(label, style: TextStyle(
        color: isFocused ? _amber : _inkMid, fontSize: 12,
      ...
      AnimatedContainer(
        ...
        decoration: BoxDecoration(
          ...
          boxShadow: isFocused
              ? [BoxShadow(color: _amber.withOpacity(0.12),
              blurRadius: 12, offset: const Offset(0, 4))]
              : [],
        ),
        child: Row(children: [
          Padding(padding: const EdgeInsets.only(left: 14),
              child: Icon(icon, color: isFocused ? _amber : _inkLight, size: 20)),
          Expanded(child: TextField(
            controller: controller, focusNode: focusNode,
            obscureText: obscure, keyboardType: keyboardType,
            style: const TextStyle(color: _inkDark, fontSize: 14.5, height: 1.4),
            decoration: InputDecoration(
              hintText: hint,
              hintStyle: const TextStyle(color: _inkLight, fontSize: 13.5),
