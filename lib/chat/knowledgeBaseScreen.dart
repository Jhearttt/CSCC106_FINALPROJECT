...
class _KnowledgeBaseScreenState extends State<KnowledgeBaseScreen> {
  ...
  final List<String> _filters = ['All', 'PDF', 'Doc', 'Link', 'Video', 'Mine'];
  ...
  Stream<QuerySnapshot> _getStream() =>
      FirebaseFirestore.instance.collection('knowledge').snapshots();
  ...
  List<DocumentSnapshot> _applyFilters(List<DocumentSnapshot> docs) {
    return docs.where((doc) {
      ...
      if (_selectedFilter == 'Mine') return uploaderId == widget.currentUserId;
    ...
    return Center(
      child: Column(mainAxisSize: MainAxisSize.min, children: [
        Container(
          ...
          child: const Icon(Icons.menu_book_rounded, size: 36, color: _kAccent),
        ),
        ...
        const Text('Tap the upload button to share\nsomething with the community',
  ...
  Future<void> _deleteMaterial(BuildContext context) async {
    final confirm = await showDialog<bool>(
      ...
      builder: (_) => AlertDialog(
        ...
        content: const Text('This will permanently remove this material.'),
        actions: [
          TextButton(
      ...
      child: ListTile(
        ...
        title: Text(title,
            style: const TextStyle(
                fontWeight: FontWeight.w700, fontSize: 14, color: _kInk),
            maxLines: 1,
        ...
        // Only show delete button for owner — no download button
        trailing: isOwner
            ? IconButton(
